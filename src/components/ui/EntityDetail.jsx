  ...
  const renderValue = useCallback(
    (field) => {
      ...
      if (valueTransforms[fieldName]) {
        return (
          <span className="text-gray-300">
            {valueTransforms[fieldName](raw, field, data)}
          </span>
        );
      }
      ...
      switch (field.type) {
        case 'id':
          return raw ? (
            <Tag color="purple">#{raw}</Tag>
          ) : (
            <span className="text-gray-500">--</span>
          );

        case 'tag':
          const tagText = field.transform
            ? field.transform(raw)
            : (raw ?? 'N/A');
          const tagColor =
            typeof field.tagColor === 'function'
              ? field.tagColor(raw)
        ...
        case 'file':
          ...
          const fileUrl = raw.startsWith('http')
            ? raw
            : `https://www.sealfall25.somee.com${raw}`;
          return (
            <Space>
              <Button
                size="small"
  ...
  const renderFieldBlock = useCallback(
    (field, keyPath = []) => {
      if (field.type === 'column') {
        ...
        if (
          rawItems.length === 2 &&
          Array.isArray(rawItems[0]) &&
          Array.isArray(rawItems[1])
        ) {
          [left, right] = rawItems;
        } else {
  ...
  return (
    <div className="min-h-full bg-white/5 p-6 rounded-xl">
      <div className="m-auto w-full">
        ...
        {children && <div className="mt-8 mx-6">{children}</div>}
      </div>
    </div>
  );
};
