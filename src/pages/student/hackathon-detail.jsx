...
import {
  useGetMyHackathonRegistrations,
  useRegisterHackathon,
} from '../../hooks/student/hackathon-registration';
...
const StudentHackathonDetail = () => {
  ...
  const phase1 = phases.find(
    (p) => p.phaseNumber === 1 || p.phaseId === phases[0]?.phaseId,
  );
  ...
  const availableTeams = useMemo(() => {
    ...
    return teamsArray.filter((t) => {
      ...
      const teamLeaderId =
        t.teamLeaderId || t.leaderId || t.leader?.id || t.leader?.userId;

      // Check if user is leader
      const isLeader =
        (teamLeaderName && userName && teamLeaderName === userName) ||
        (teamLeaderId && userId && teamLeaderId === userId);

      // Check if team hasn't joined any hackathon (hackathonId is null)
      const hasNotJoined =
        t.hackathonId === null ||
        t.hackathonId === undefined ||
        t.hackathonName === '(No hackathon)' ||
        t.hackathonName === null;
  ...
  const myRegistration = useMemo(() => {
    ...
    return (
      myRegistrations.find((reg) => reg.hackathonId === parseInt(id)) || null
    );
  }, [myRegistrations, id]);
  ...
  const [mentorSelectModalVisible, setMentorSelectModalVisible] =
    useState(false);
  ...
  const { data: approvedMentorsData, isLoading: mentorsLoading } =
    useGetApprovedMentorsByHackathon(id, {
      enabled: !!id && mentorSelectModalVisible,
    });

  const approvedMentors = Array.isArray(approvedMentorsData)
    ? approvedMentorsData
    : approvedMentorsData?.data
      ? approvedMentorsData.data
      : [];
  ...
  const filteredMentors = useMemo(() => {
    if (!mentorSearchQuery.trim()) return approvedMentors;
    const query = mentorSearchQuery.toLowerCase();
    return approvedMentors.filter((mentor) => {
      const username =
        mentor.username || mentor.userName || mentor.fullName || '';
      const email = mentor.email || '';
      const chapterName = mentor.chapterName || '';
      const position = mentor.position || '';
      return (
  ...
  }, [mentorSearchQuery, approvedMentors]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {groupsData.map((group) => (
                    <div
                      ...
                      onClick={() =>
                        navigate(
                          `/student/hackathons/${id}/phases/${phase1.phaseId}`,
                        )
                      }
                    >
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-card-background border border-card-border backdrop-blur-xl">
            ...
            >
              {myRegistration &&
                (() => {
                  const statusInfo = getMyRegistrationStatusTag(
                    myRegistration.status,
                  );
                  if (!statusInfo) return null;
                  const isWaitingMentor =
                    myRegistration.status?.toLowerCase() === 'waitingmentor';
                  return (
                    <div className="mb-2">
              ...
              {myRegistration &&
                (myRegistration.status?.toLowerCase() === 'approved' ||
                  myRegistration.status?.toLowerCase() === 'waitingmentor') &&
                registration?.status?.toLowerCase() === 'approved' &&
                registration.selectedPhaseId &&
                registration.selectedTrackId && (
                  <>
                    <Button
                      size="large"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        navigate(
                          `/student/hackathons/${id}/mentor-registration`,
                        )
                      }
                      className="w-full"
                    >
          ...
          columns={[
            {
              ...
              render: (rank) => {
                if (rank === 1)
                  return <TrophyOutlined className="text-yellow-400 text-xl" />;
                if (rank === 2)
                  return <TrophyOutlined className="text-gray-400 text-xl" />;
                if (rank === 3)
                  return <TrophyOutlined className="text-amber-600 text-xl" />;
      ...
      >
        ...
        >
          <Form.Item
            ...
            help={
              availableTeams.length === 0
                ? 'Bạn chưa có đội nào là đội trưởng và chưa tham gia hackathon. Vui lòng tạo đội trước.'
                : null
            }
          >
            <Select
              ...
              options={availableTeams.map((team) => ({
                ...
                label:
                  team.teamName ||
                  team.name ||
                  `Đội ${team.teamId || team.id || team.teamID}`,
              }))}
      ...
      <Modal
        ...
        okButtonProps={{
          ...
          className:
            'bg-gradient-to-r from-green-500 to-emerald-400 hover:from-green-600 hover:to-emerald-500 text-white border-0',
        }}
        width={700}
        className="[&_.ant-modal-content]:bg-gray-900/95 [&_.ant-modal-content]:backdrop-blur-xl [&_.ant-modal-header]:border-white/10 [&_.ant-modal-body]:text-white [&_.ant-modal-close]:text-white [&_.ant-modal-mask]:bg-black/50"
      >
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-base">
                {mentorSearchQuery.trim()
                  ...
                  : 'Chưa có mentor nào được duyệt cho hackathon này'}
              </p>
              {mentorSearchQuery.trim() && (
